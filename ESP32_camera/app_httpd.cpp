...
static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results)
{
    ...
    if (enrolled_count < FACE_ID_SAVE_NUMBER && is_enrolling){
        ...
        rgb_printf(fb, FACE_COLOR_CYAN, "  ", id);
    }

    face_info_t recognize = recognizer.recognize(tensor, landmarks);
    if(recognize.id >= 0){
        rgb_printf(fb, FACE_COLOR_GREEN, "Comms Expert", recognize.id, recognize.similarity);
    } else {
        rgb_print(fb, FACE_COLOR_RED, "Imposter Suspect");
    }
    return recognize.id;
}
#endif
